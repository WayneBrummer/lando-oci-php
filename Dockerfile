...
LABEL maintainer="Wayne Brummer <waynebrummer101@gmail.com>"
...
RUN wget http://php.net/distributions/php-7.2.19.tar.gz && \
    mkdir php_oci && \
    mv php-7.2.19.tar.gz ./php_oci
WORKDIR php_oci
RUN tar xfvz php-7.2.19.tar.gz
WORKDIR php-7.2.19/ext/pdo_oci
RUN phpize && \
